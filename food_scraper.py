...
def download_csvs_job(food_ids,thread_id):
  ...
  downloaded_ids = set([int(filename[:filename.index('.')]) for filename in os.listdir('csvs')])
...
def parse_csvs():
  ...
  for filename in os.listdir('csvs'):
    food_obj = parse_csv(os.path.join('csvs',filename))
